import { AuditResponse } from "./gemini";
...
export function generatePressReport(audit: AuditResponse): string {
  const { meta, riskAssessment, causalMap, chronology, legalMatrix, auditIntegrity, documentationGaps, escalationPlan } = audit;
  ...
  const kpis = [
    ...
    { value: auditIntegrity.flagsRaised.length, suffix: "", label: { cz: "Kritické flagy", en: "Critical Flags" } }
  ];

  const simulatedLogs = [
    { t: { cz: "> Inicializace sémantického jádra LEX FORENSICA v7.0...", en: "> Initializing LEX FORENSICA v7.0 semantic core..." }, d: 0 },
    ...
    <nav class="fixed top-0 w-full bg-white/80 backdrop-blur-md border-b z-50 px-6 py-4 flex justify-between items-center no-print">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Auditní protokol v7.0</p>
            </div>
        </div>
...
export function generateCSVExport(audit: AuditResponse): string {
  ...
  rows.push(["SECTION", "TYPE/VIOLATION", "SYSTEM_VERSION/DOMAIN", "SUBJECT_VERSION/REASONING", "SEVERITY", "DATE/ID"]);
