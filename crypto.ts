    ...
    <Document>
        ...
        <View style={styles.section}>
          ...
          {audit.discrepancyMatrix?.map((d, i) => (
            <View key={i} style={styles.discrepancyBox}>
              <Text style={[styles.text, styles.bold]}>{d.shortLabel || 'Diskrepance'} (Závažnost: {d.severity})</Text>
              <Text style={styles.text}><Text style={styles.bold}>Čas:</Text> {d.time}</Text>
              <Text style={styles.text}><Text style={styles.bold}>Systém:</Text> {d.claimA}</Text>
              <Text style={styles.text}><Text style={styles.bold}>Subjekt:</Text> {d.claimB}</Text>
              <Text style={styles.text}><Text style={styles.bold}>Důkaz/Argument:</Text> {d.evidence}</Text>
            </View>
          ))}
        </View>

        <View style={styles.footer}>
          <Text>LEX FORENSICA v8.0 — Generováno automaticky forenzním auditním enginem. Dokument slouží jako podklad pro právní obranu.</Text>
        </View>
      </Page>
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <Text style={[styles.text, styles.bold]}>Defense Action Tier 0 (Okamžitá opatření):</Text>
          {audit.escalationPlan?.tierActions?.defense_action_tier_0?.map((a, i) => <Text key={i} style={styles.text}>• {a}</Text>)}
          <Text style={[styles.text, styles.bold, { marginTop: 5 }]}>Defense Action Tier 1 (Právní kroky):</Text>
          {audit.escalationPlan?.tierActions?.defense_action_tier_1?.map((a, i) => <Text key={i} style={styles.text}>• {a}</Text>)}
          <Text style={[styles.text, styles.bold, { marginTop: 5 }]}>Defense Action Tier 2 (Soudní napadení):</Text>
          {audit.escalationPlan?.tierActions?.defense_action_tier_2?.map((a, i) => <Text key={i} style={styles.text}>• {a}</Text>)}
          <Text style={[styles.text, styles.bold, { marginTop: 5 }]}>Defense Action Tier 3 (Mezinárodní instance):</Text>
          {audit.escalationPlan?.tierActions?.defense_action_tier_3?.map((a, i) => <Text key={i} style={styles.text}>• {a}</Text>)}
        </View>
        ...
        <View style={styles.footer}>
          <Text>LEX FORENSICA v8.0 — Strana 2 | Důvěrný forenzní materiál</Text>
        </View>
      </Page>
    </Document>
  );
