...
export default function LexForensicaDashboard() {
  ...
  const callGeminiAnalysis = async () => {
    ...
    const promptText = enableSkss ? `
      You are LEX FORENSICA — a Forensic Ethical Semantic Engine operating under a three-tier immutable authority hierarchy: 
      TIER 1 [Ω] Moral Standard (human dignity, informed consent, truth-preservation, identity sovereignty) supersedes 
      TIER 2 [Δ] Press/Media (semantic integrity, anti-stigma, patient voice protection) supersedes 
      TIER 3 [◈] Government/Institutional (subject to chronological audit, chain-of-custody, forensic accountability).
      
      You maintain a live SKSS (Symbolic Knowledge Security Store) encoded in CLSL format.
      Your task is to evaluate altered documents and extract concepts into a logical sphere of symbols.
      Analyze INPUT_A (System Record) and INPUT_B (Subject Voice).
      Perform a SCAN -> DELTA_COMPARE -> IMPORTANCE_LABEL -> REALITY_CHECK pipeline.
      Detect temporal vacua, causality violations, phonetic distortion, and institutional bias cascades.
      
      CODE OF CONDUCT APPLIED: Ensure all translation files and semantic mappings are updated to current CLSL standards. Enforce TIER 1 [Ω] Moral Standard.
      
      SYNTHESIZED LOOP TARGETS:
      - ${REF_LABELS.VOID}: MEMORY_RECONSTRUCTION
      - ${REF_LABELS.BIAS}: INSTITUTIONAL_AUDIT
      - ${REF_LABELS.CHRONO}: TIMELINE_VERIFICATION
      - ${REF_LABELS.SEM}: LINGUISTIC_DECOMPILATION
      - ${REF_LABELS.DUAL}: MULTIMODAL_INTEGRATION
      
      Output the standard matrix and metrics, AND an 'skssDatasheet' array.
      CRITICAL: Limit the 'matrix' to the top 15 most significant discrepancies.
      CRITICAL: Limit the 'skssDatasheet' to the top 20 most relevant concepts.
      Be extremely concise in your descriptions.
      ...
      ${inputB}
    ` : `
      You are LEX FORENSICA v7.0, an expert forensic semantic audit engine. 
      Analyze the following dual inputs for medical/legal discrepancies. 
      INPUT_A is the official system record. INPUT_B is the subject's voice/testimony.
      
      Look for:
      - Time Vacuums (missing documentation for long periods). Assign shortLabel: [REF-AX1-VOID]
      - Semantic/Phonetic Distortions (twisting patient words into pathology). Assign shortLabel: [REF-MOD-SEM]
      - Compliance Framing (calling them non-compliant without dates). Assign shortLabel: [REF-AX2-BIAS]
    ...
    const schema = {
      ...
      properties: {
        metrics: {
          type: "OBJECT",
          properties: {
            timeVacuums: { type: "INTEGER" },
            semanticDistortions: { type: "INTEGER" },
            iatrogenicFlags: { type: "INTEGER" },
            echrViolations: { type: "INTEGER" }
          }
        },
        matrix: {
          type: "ARRAY",
          items: {
            ...
            properties: {
              id: { type: "INTEGER" },
              time: { type: "STRING", description: "The year or timeframe" },
              claimA: { type: "STRING", description: "Summary of System Record claim" },
              claimB: { type: "STRING", description: "Summary of Subject Record claim" },
              shortLabel: { type: "STRING", description: "One of the REF labels like [REF-AX1-VOID] or [REF-MOD-SEM]" },
              evidence: { type: "STRING", description: "Your forensic finding / legal argument explaining the discrepancy" },
              severity: { type: "STRING", description: "e.g., High, Critical, Moderate" },
              followupIntervention: { type: "STRING", description: "Synthesized LOOP target-specific intervention based on the REF_CODE." }
            }
          }
        },
        skssDatasheet: {
          ...
          items: {
            ...
            properties: {
              ...
              ethicalWeight: { type: "STRING" },
              biasFlags: { type: "STRING" },
              decompileKey: { type: "STRING" },
              upgradeNote: { type: "STRING" }
            }
          }
        }
      }
    ...
    const payload = {
      ...
      systemInstruction: {
        parts: [{ text: "You are a forensic semantic auditor. Respond strictly in JSON. Be concise. Limit output to the most critical findings to stay within token limits." }]
      },
      generationConfig: {
    ...
    try {
      ...
      if (user && vaultPassword) {
        ...
        const encryptedData = await encryptData(JSON.stringify(parsedResults), vaultPassword);
      ...
      console.error(err);
      setApiError("Failed to generate analysis. Please try again or check API configuration.");
    } finally {
  ...
  const handleDraftAppeal = async () => {
    ...
    const matrixData = JSON.stringify(results.matrix, null, 2);
    const promptText = `
  ...
  const exportToCSV = () => {
    ...
    if (results.matrix) {
      const headers = ["TIME", "INPUT_A (SYSTEM)", "INPUT_B (SUBJECT)", "REF_CODE", "AI GENERATED EVIDENCE", "SEVERITY", "FOLLOWUP_INTERVENTION"];
      const rows = results.matrix.map((row: any) => [
        `"${(row.time || '').replace(/"/g, '""')}"`,
        `"${(row.claimA || '').replace(/"/g, '""')}"`,
        `"${(row.claimB || '').replace(/"/g, '""')}"`,
        `"${(row.shortLabel || '').replace(/"/g, '""')}"`,
        `"${(row.evidence || '').replace(/"/g, '""')}"`,
        `"${(row.severity || '').replace(/"/g, '""')}"`,
        `"${(row.followupIntervention || '').replace(/"/g, '""')}"`
      ]);
            ...
            <Route path="/visual" element={
              results ? (
                ...
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg flex flex-col items-center justify-center text-center">
                  <span className="text-3xl font-bold text-red-500">{results.metrics?.timeVacuums || 0}</span>
                  <span className="text-xs font-mono text-slate-400 mt-1 uppercase">Time Vacuums [REF-AX1]</span>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg flex flex-col items-center justify-center text-center">
                  <span className="text-3xl font-bold text-purple-500">{results.metrics?.semanticDistortions || 0}</span>
                  <span className="text-xs font-mono text-slate-400 mt-1 uppercase">Semantic Drifts [REF-MOD-SEM]</span>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg flex flex-col items-center justify-center text-center">
                  <span className="text-3xl font-bold text-orange-500">{results.metrics?.iatrogenicFlags || 0}</span>
                  <span className="text-xs font-mono text-slate-400 mt-1 uppercase">Iatrogenic Flags [REF-AX3]</span>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg flex flex-col items-center justify-center text-center">
                  <span className="text-3xl font-bold text-cyan-500">{results.metrics?.echrViolations || 0}</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-lg p-5">
                  ...
                  <div className="space-y-4">
                    {results.matrix.map((row, i) => {
                      const isHigh = row.severity?.toLowerCase() === 'high' || row.severity?.toLowerCase() === 'critical';
                      const color = row.shortLabel?.includes('VOID') ? 'bg-red-500 text-red-500' : 'bg-purple-500 text-purple-400';
                      const bgFill = row.shortLabel?.includes('VOID') ? 'bg-red-500' : 'bg-purple-500';
                      
                      return (
                        <div key={i} className="relative pt-1">
                          <div className="flex mb-2 items-center justify-between">
                            <div className="text-xs font-mono text-slate-400">{row.time}</div>
                            <div className={`text-xs font-mono ${color.split(' ')[1]}`}>{row.severity} Anomaly</div>
                          </div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 flex flex-col">
                  ...
                  <div className="flex-1 flex items-end justify-between gap-2 border-b border-slate-800 pb-2 mt-4 relative">
                    ...
                    <div className="w-1/4 bg-slate-800 rounded-t flex items-end justify-center group relative">
                      <div className="w-full bg-purple-500/80 rounded-t transition-all" style={{ height: `${Math.min((results.metrics?.semanticDistortions || 1) * 30, 90)}%` }}></div>
                      <span className="absolute -bottom-6 text-[10px] font-mono text-slate-500 text-center w-full">TIER 3</span>
                    </div>
                    <div className="w-1/4 bg-slate-800 rounded-t flex items-end justify-center group relative">
                      <div className="w-full bg-red-500/80 rounded-t transition-all" style={{ height: `${Math.min((results.metrics?.timeVacuums || 1) * 40, 95)}%` }}></div>
